...
Details for configuring, checking, updating, and removing Astra Orchestrator.
...
`@deepseek-ai/dsh` 0.1.5-rc.1), Node 20.10+, configured DeepSeek API access, and a ChatGPT Plus/Pro subscription
with Codex access to Astra.
...
In this DSH version the bundle composition can set the root provider/model, but
reasoning effort is read from settings. Use `--activate` in either install mode
to write the xhigh default; adding `reasoningEffort` to the bundle row alone
would not configure it.
...
entry's reasoning effort to xhigh**, so it discards a custom root effort:
...
node install.mjs --activate           # flip the default model (and reset its effort to xhigh)
...
activation and realistic tasks. No measured speed, cost, or quality improvement is claimed. The separate
[cost and performance estimates](cost-and-performance.md) use explicit hypothetical workloads.

## Verifying the topology

After a reinstall, restart the host when it is idle and open a fresh session;
then check the effective route, reasoning effort, and the child tool catalog rather than assuming a
hot reload.
...
3. The GUI shows each child session with its own route; workers read
   ...
   gpt-6-astra` at high. The lead should show Astra at xhigh after activation.
...
- **`--activate` resets the root effort.** It rewrites `agent-default-model` to
  `openai-codex/gpt-6-astra` at xhigh, discarding a custom effort. Use plain
